...
// Función para obtener las horas restantes de la materia
function obtenerHorasTotalesMateria($conexion, $idMaestroMateria) {

    $query = "SELECT m.Horas_totales
    FROM Materia m
    JOIN MaestroMateria mm ON m.ID_Materia = mm.ID_Materia
    WHERE mm.id_maestro_materia = '$idMaestroMateria'";
            ...
            if ($idMaestroMateria !== '') {
                if (isset($horaInicio[$indice]) && isset($horaFin[$indice])) {
                    ...
                    $horaFinActual = $horaFin[$indice];
                        ...
                        $horasTotales = obtenerHorasTotalesMateria($conexion, $idMaestroMateria);
                        if ($horasRestantes >= $horasTotales) {
                            throw new Exception("La materia " . obtenerNombreMateria($conexion, $idMaestroMateria) . " excedio las horas totales.");
                        }
